<script lang="ts">
  import {
    A,
    Button,
    Dropdown,
    DropdownItem,
    Input,
    Kbd,
    Toggle,
    tooltip,
    Tooltip
  } from 'flowbite-svelte';
  import { ChevronDownOutline } from 'flowbite-svelte-icons';
  import { twMerge } from 'tailwind-merge';
  import { browser } from 'wxt/browser';
  import Icon from '@iconify/svelte';

  import config from '@/lib/config';
  import shortcut from '@/lib/shortcut';
  import type { FeatureField } from '@/lib/types';
  import ProviderIcon from '@/components/ProviderIcon.svelte';
  import ProvidersDropdown from '@/components/ProvidersDropdown.svelte';

  interface Prop {
    title?: string;
    field: FeatureField;
    showFreeProviders?: boolean;
    buttonColor?: string;
  }

  const { title, field, showFreeProviders = false, buttonColor }: Prop = $props();
</script>

<div class="grid grid-cols-[144px_1fr] items-center gap-2">
  {#if buttonColor}
    <Button
      color={buttonColor}
      size="xs"
      class="flex items-center border-none px-1 py-1.5 font-medium shadow">
      <span>{title}</span>
      {#if $config[field].shortcut?.length}
        <span>
          ({shortcut.formatForDisplay($config[field].shortcut).join('')})
        </span>
      {/if}
    </Button>
  {:else}
    <div class="flex items-center gap-2 text-xs font-medium">
      <span>{title}</span>
      {#if $config[field].shortcut?.length}
        <span>
          {shortcut.formatForDisplay($config[field].shortcut).join('')}
        </span>
      {/if}
    </div>
  {/if}
  <button
    type="button"
    class="flex w-full gap-1 rounded-lg border-none bg-slate-100 p-1 text-slate-900 shadow hover:bg-slate-200/70 dark:bg-slate-700 dark:text-slate-100 hover:dark:bg-slate-600">
    <div class="flex flex-1 items-center gap-1">
      <ProviderIcon provider={$config[field].provider} class="w-4" />
      {#if $config[field].provider}
        <span class="line-clamp-1 w-full text-left">
          {$config[field].provider.name}
          {#if $config[field].provider.type !== 'free' && $config[field].provider.model}
            ({$config[field].provider.model})
          {/if}
        </span>
      {:else}
        <span class="line-clamp-1 w-full text-slate-300 dark:text-slate-500">
          {'-'.repeat(6)}
        </span>
      {/if}
    </div>
    <!-- <ChevronDownOutline class="h-5 w-5 text-slate-400" /> -->
    <Icon icon="tabler:chevron-down" class="h-4 w-4 shrink-0 text-slate-400" />
  </button>
  <ProvidersDropdown {field} {showFreeProviders} class="max-h-56" />
</div>
