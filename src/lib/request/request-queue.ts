...
export class RequestQueue {
  ...
  private async executeTask(task: RequestTask & { hash: string }) {
    logger.info(
      `🏃 Starting execution of task ${task.id} (attempt ${task.retryCount + 1}) at ${Date.now()}`
    );
    ...
    try {
      ...
      if (task.retryCount < this.options.maxRetries) {
        ...
        logger.warn(
          `🔄 Retrying task ${task.id} (attempt ${task.retryCount}/${this.options.maxRetries}) after ${Math.round(delayMs)}ms`
        );
        ...
        logger.error(
          `💀 Task ${task.id} failed permanently after ${this.options.maxRetries} retries`
        );
        task.reject(error);
      }
    } finally {
  ...
  private refillTokens() {
    ...
    //   logger.debug(`🪣 Token bucket refilled: ${oldTokens.toFixed(2)} -> ${this.bucketTokens.toFixed(2)} (+${tokensToAdd.toFixed(2)}) after ${timeSinceLastRefill}ms`)
    // }
