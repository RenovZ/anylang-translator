<script lang="ts">
  ...
  let arrowParams = $derived<{ placement: Placement; cords: Partial<Coords> }>({
    placement: 'top',
    cords: { x: 0, y: 0 }
  });
  ...
  function set_triggers(node: HTMLElement) {
    ...
    if (triggeredBy)
      triggerEls = Array.from(node.ownerDocument.querySelectorAll<HTMLElement>(triggeredBy));
  ...
  function closeOnClickOutside(event: MouseEvent) {
    ...
    if (!isClickInsidePopover && !isClickOnTrigger && closeOnClickOutsideCallback()) {
      close_popover(event);
      isOpen = false;
    }
  }
