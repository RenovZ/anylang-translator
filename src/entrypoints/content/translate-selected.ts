...
export class TranslateSelected {
  ...
  public initialize(
    ...
    lang: { fixTLanguageCode(code: string): string | null; codeToLanguage(code: string): string; isRtlLanguage(code: string): boolean },
    ...
    void config.onReady(async () => {
      ...
      this.alwaysTranslateThisSite = config.get<string[]>("alwaysTranslateSites").indexOf(this.tabHostName) !== -1;
      this.translateThisSite = config.get<string[]>("neverTranslateSites").indexOf(this.tabHostName) === -1;
      ...
      this.dontShowIfSelectedTextIsTargetLang = config.get<string>("dontShowIfSelectedTextIsTargetLang");
      ...
      pageTranslator.onGetOriginalTabLanguage((tabLanguage) => {
        ...
        this.translateThisLanguage = config.get<string[]>("neverTranslateLangs").indexOf(tabLanguage) === -1;
      ...
      config.onChanged((name, value) => {
        if (name === "textTranslatorService") {
          ...
          this.alwaysTranslateThisSite = Array.isArray(value) && value.map((item) => String(item)).indexOf(this.tabHostName) !== -1;
          this.updateEventListeners();
        } else if (name === "neverTranslateSites") {
          this.translateThisSite = !(Array.isArray(value) && value.map((item) => String(item)).indexOf(this.tabHostName) !== -1);
          this.updateEventListeners();
        } else if (name === "neverTranslateLangs") {
          this.translateThisLanguage = !(Array.isArray(value) && value.map((item) => String(item)).indexOf(this.originalTabLanguage) !== -1);
          this.updateEventListeners();
  ...
  private updateEventListeners(): void {
    const shouldEnable =
      ...
      ((this.dontShowIfPageLangIsTargetLang === "yes" && this.originalTabLanguage !== this.currentTargetLanguage) ||
  ...
  private readSelection(skipIfSameText = false): boolean {
    ...
    if (skipIfSameText && this.gSelectionInfo && newSelection && this.gSelectionInfo.text === newSelection.text) {
  ...
  private async onSelectionReleased(clientX: number, clientY: number): Promise<void> {
    ...
    const passSelectedLangTarget =
      (this.dontShowIfSelectedTextIsTargetLang === "yes" && detected.lang !== this.currentTargetLanguage) ||
    ...
    const passTextValidation = this.dontShowIfIsNotValidText !== "yes" || this.isValidText(selectedText);
  ...
  private init(): void {
    ...
    if (replaceButton) {
      if (this.gSelectionInfo && (this.gSelectionInfo.isInputElement || this.gSelectionInfo.isContentEditable)) {
    ...
    for (const serviceInfo of serviceButtons) {
      this.shadowRoot.getElementById(serviceInfo.id)?.addEventListener("click", () => {
        if (serviceInfo.service === "deepl") {
          ...
          if (!confirmed) {
            const approved = window.confirm(this.i18n?.getMessage("msgSetDeepLAlert") ?? "Use DeepL?");
            if (!approved) {
              return;
            }
  ...
  private refreshPanelSelections(): void {
    ...
    Object.values(mapping).forEach((id) => this.shadowRoot?.getElementById(id)?.classList.remove("selected"));
  ...
  private updatePanelExpandState(): void {
    ...
    if (
      ...
      (this.prevSelectionInfo && (this.prevSelectionInfo.isContentEditable || this.prevSelectionInfo.isInputElement))
    ) {
  ...
  private updatePanel(result: string): void {
    if (!this.panelElement || !this.originalElement || !this.translatedElement || !this.prevSelectionInfo || !this.lang) {
      return;
    }

    this.translatedElement.setAttribute("dir", this.lang.isRtlLanguage(this.currentTargetLanguage) ? "rtl" : "ltr");
    ...
    if (this.panelElement.style.display !== "block") {
      ...
      top = Math.min(window.innerHeight - panelHeight, Math.max(0, this.prevSelectionInfo.bottom + 5));
  ...
  private async replaceSelectionWithHotTranslation(): Promise<void> {
    ...
    const parentWithFocus = selection.element.parentNode as (ParentNode & { focus?: () => void }) | null;
  ...
  private replaceCurrentSelection(result: string): void {
    ...
    if (info.isInputElement) {
      (info.element as HTMLInputElement | HTMLTextAreaElement).setSelectionRange(info.selStart, info.selEnd);
