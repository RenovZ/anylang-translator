...
<SectionRow
  ...
  <div slot="controls">
    <Button
      ...
      <span>
        {#if HOTKEYS.includes($config.quickTranslate.translate.triggerOnHover)}
          <Kbd>{HOTKEY_ICONS[$config.quickTranslate.translate.triggerOnHover]}</Kbd>
          <Kbd>{HOTKEY_EVENT_KEYS[$config.quickTranslate.translate.triggerOnHover]}</Kbd>
        {:else}
    ...
    <Dropdown simple placement="bottom-end" class="max-h-80 overflow-y-auto shadow-md">
      {#each HOTKEYS as key (key)}
        <DropdownItem onclick={() => ($config.quickTranslate.translate.triggerOnHover = key)}>
